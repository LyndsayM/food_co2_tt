...
food_consumption %>%
  ...
  scale_y_log10()+
  geom_point()+
  geom_label_repel(aes(label=food_category, size=NULL), nudge_y=-0.35, nudge_x = 0.25)+
  theme(
...
#from video
#I didn't find this helpful/meaningful
food_consumption %>%
  group_by(food_category) %>%
...
food_consumption %>% group_by(country) %>%
  ggplot(aes(x=consumption, y=co2_emission))+ 
  geom_point()+
  theme(
    axis.text.x = element_blank())
```
